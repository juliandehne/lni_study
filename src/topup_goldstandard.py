...
def main() -> None:
    ...
    parser.add_argument("--model", default=preflight.DEFAULT_MODEL,
                        help=f"SAIA model (default {preflight.DEFAULT_MODEL}). Only "
                             "its FAMILY has to match the existing goldconfirm "
                             "checkpoint, since that is what names the file; a version "
                             "bump within the family keeps appending to the same "
                             "checkpoint and is recorded per row in its `model` column.")

...
import preflight  # noqa: E402  (fail-fast SAIA + path checks before the corpus scan)
...
def main() -> None:
    ...
    # Fail-fast PATH preflight BEFORE the (slow, mount-bound) corpus scan: the
    # corpus folder and the data-root subtree must be present. The SAIA check is
    # deferred to the annotation step below, because the report-only / estimate
    # modes return early and don't need a token.
    preflight.require(
        [preflight.check_path(lni_folder, label="lni_folder")]
    ...
    # Fail-fast SAIA preflight: reachable + token accepted, before annotating.
    preflight.require([preflight.check_saia(base_url, saia_api_key)])
